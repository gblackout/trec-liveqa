...
print('=====>>> Number of selected medications is {}, maximum number of sampels is {}'.format(len(medications), max_num_samples))
...
print('=====>>> Number of selected medications is {}, maximum number of samples with admission medications is {}'.format(len(admis_meds), max_num_admisMedNotes))
...
for file in files:
    ...
    if patient['discharge medications']:
        note_meds = get_med(patient['discharge medications'], numerical_indexed=is_number_index(patient['discharge medications']))
        note_vec = sorted([medications[med] for med in note_meds if med in medications])
        if not note_vec: continue
...
for file in files:
    ...
    if patient['medications on admission']:
        note_meds = get_med(patient['medications on admission'], numerical_indexed=is_number_index(patient['medications on admission']))
        note_vec = sorted([medications[med] for med in note_meds if med in medications])
        if not note_vec: continue
...
print('=====>>> mean of jaccard scores is {}, number of samples is {}'.format(np.mean(jaccard_scores.values()), len(jaccard_scores)))
print('=====>>> mean of jaccard scores1 is {}, number of samples is {}'.format(np.mean(jaccard_scores1.values()), len(jaccard_scores1)))